...
plot1 <- data_plot1 %>%
  ...
  annotate(geom = "text", x = 1, y = 0.62, size = 3,
           label = paste0(round(data_plot1$proportion[data_plot1$grade == 2 & data_plot1$difficulty_summarized == 1] * 100),
                          "% chose")) +
  annotate(geom = "text", x = 2, y = 0.59, size = 3,
           label = "the medium level") +
  annotate(geom = "text", x = 2, y = 0.54, size = 3,
           label = paste0(round(data_plot1$proportion[data_plot1$grade == 3 & data_plot1$difficulty_summarized == 1] * 100),
                          "% chose")) +
  annotate(geom = "text", x = 3, y = 0.53, size = 3,
           label = "the medium level") +
  annotate(geom = "text", x = 3, y = 0.48, size = 3,
           label = paste0(round(data_plot1$proportion[data_plot1$grade == 4 & data_plot1$difficulty_summarized == 1] * 100),
                          "% chose")) +
...
model_q_score <- lm(q_score_summarized ~ difficulty_summarized * grade, data = data_model)
summary(model_q_score) # only grade is a significant predictor, not their chosen difficulty level

model_login_grade <- lm(login_count ~ difficulty_summarized * grade, data = data_model)
summary(model_login_grade)
summary(lm.beta(model_login_grade))

model_diff<- lm(difficulty_summarized ~ login_count * grade, data = data_model)
summary(model_diff)
summary(lm.beta(model_diff))

model_login_only <- lm(login_count ~ difficulty_summarized, data = data_model)
summary(model_login_only)
summary(lm.beta(model_login_only))

model_diff_only <- lm(q_score_summarized ~ difficulty_summarized, data = data_model)
summary(model_diff_only)
summary(lm.beta(model_diff_only))
